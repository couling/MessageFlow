...
from .type_encoder_decoders import StructEncoderDecoder
...
class DecoderContext:
    ...
    def decode_variable_int(self) -> int:
        ...
        if first_byte[0] & 0xC0 == 0x80:
            return int.from_bytes(first_byte + self.read(1), constants.ENDIAN)
        if first_byte[0] & 0xE0 == 0xC0:
            return int.from_bytes(first_byte + self.read(3), constants.ENDIAN)
        if first_byte[0] & 0xF0 == 0xE0:
            return int.from_bytes(first_byte + self.read(7), constants.ENDIAN)
        raise exceptions.ParseError(f"Invalid first byt for variable int {first_byte.hex()}")
